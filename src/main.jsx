...
const filtered=useMemo(()=>activeProducts.filter(p=>(cat==='All'||p.cat===cat)&&p.name.toLowerCase().includes(search.toLowerCase())),[activeProducts,cat,search]);
 ...
 <header><div className="nav container">
   <button className="hamb" onClick={()=>setMenu(!menu)}>☰</button><a className="logo" href="#home"><span>RTC</span><small>TECH</small></a>
 ...
 <main id="home">
  <section className="hero"><div className="hero-inner container"><div className="hero-copy"><span className="eyebrow">RTC · LAHORE</span><h1>Technology<br/><em>upgraded.</em></h1><p>Discover smart wearables, immersive audio, creator gear and everyday tech — selected to keep you connected.</p><div className="hero-buttons"><a href="#shop" className="btn primary">Shop the collection <span>→</span></a><a href="#categories" className="btn ghost">Explore categories</a></div><div className="hero-meta"><span>✓ Curated products</span><span>✓ Nationwide delivery</span><span>✓ Easy support</span></div></div><div className="hero-art"><div className="orb"></div><img src={dbProducts[0]?.img} alt="RTC smartwatch"/><div className="floating"><small>FEATURED</small><b>{dbProducts[0]?.name || "Loading..."}</b><span>{dbProducts[0] ? money(dbProducts[0].price) : ""}</span></div></div></div></section>
  ...
  <section id="deals" className="deal"><div className="container deal-inner"><div><span className="eyebrow">LIMITED-TIME PICKS</span><h2>Smart tech.<br/><em>Smart prices.</em></h2><p>Upgrade your setup with standout gadgets from the RTC collection.</p><a className="btn light" href="#shop">Shop deals →</a></div><div className="deal-products">{activeProducts.slice(1,4).map(p=><Product key={p.id} p={p} add={add} wish={wish} toggleWish={toggleWish} compact/>)}</div></div></section>
  ...
  <section id="about" className="about"><div className="container about-grid"><div><span className="eyebrow">THE RTC STANDARD</span><h2>More than gadgets.<br/><em>Better experiences.</em></h2></div><div><p>RTC brings together practical technology, personal audio, smart wearables, gaming accessories and creator essentials under one modern shopping experience.</p><p>From our Lahore base, we serve customers across Pakistan with a product-first approach and straightforward support.</p><a href="#contact" className="text-link">Talk to RTC →</a></div></div></section>
  <section className="newsletter"><div className="container newsletter-inner"><div><span className="eyebrow">STAY IN THE LOOP</span><h2>New tech, fresh deals.</h2></div><form onSubmit={e=>{e.preventDefault();setNotice('Thanks — you are on the RTC list.')}}><input type="email" required placeholder="Your email address"/><button className="btn primary">Subscribe →</button></form></div></section>
 </main>
 <footer id="contact"><div className="container footer-grid"><div><a className="logo footer-logo" href="#home"><span>RTC</span><small>TECH</small></a><p>Modern technology and everyday gadgets, curated for Pakistan.</p></div><div><h4>Explore</h4><a href="#shop">Shop</a><a href="#categories">Categories</a><a href="#deals">Deals</a><a href="#about">About us</a></div><div><h4>Support</h4><a href="#contact">Contact us</a><a href="#">Track order</a><a href="#">Shipping policy</a><a href="#">Returns</a></div><div><h4>Contact</h4><p>+92 321 3232000</p><p>contact@rcmmultimedia.com</p><p>Usama Center, Hall Road<br/>Lahore, Pakistan</p></div></div><div className="copyright container"><span>© 2026 RTC. All rights reserved.</span><span>Designed for a premium shopping experience.</span></div></footer>
 {quick&&<div className="modal-back" onClick={()=>setQuick(null)}><div className="modal" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setQuick(null)}>×</button><img src={quick.img}/><div><span className="eyebrow">QUICK VIEW</span><h2>{quick.name}</h2><p className="price">{money(quick.price)}</p><p>Quality-selected tech from the RTC collection.</p><button className="btn primary" onClick={()=>{add(quick);setQuick(null)}}>Add to cart →</button></div></div></div>}
